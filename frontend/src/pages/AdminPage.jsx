      ...
      <div className="mt-6 space-y-4">
        {shops.map((shop) => (
          <article key={shop.id} className="rounded-lg border border-zinc-200 bg-white p-5 shadow-sm">
            ...
            {shop.payment_issues?.length > 0 && <div className="mt-4 rounded border border-red-300 bg-red-50 p-3 text-sm text-red-900"><p className="font-semibold">Payment issues requiring review</p>{shop.payment_issues.map((issue) => <p key={`${issue.kind}-${issue.id}`} className="mt-1">{issue.kind}: ${Number(issue.amount_cents / 100).toFixed(2)} · {issue.status.replaceAll("_", " ")}{issue.reason ? ` · ${issue.reason}` : ""}{issue.due_by ? ` · response due ${new Date(issue.due_by).toLocaleString()}` : ""}</p>)}</div>}
